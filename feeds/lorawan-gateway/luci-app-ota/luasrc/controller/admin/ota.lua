...
function index()
  ...
  entry({"admin", "gateway", "ota"}, alias("admin", "gateway", "ota", "index"), _("OTA"), 50)
  entry({"admin", "gateway", "ota", "index"}, post_on({ apply = "1" }, "action_ota"), _("OTA"), 1)
  entry({"admin", "gateway", "ota", "config"}, cbi("admin_system/ota_config"), _("Configuration"), 2)
  entry({"admin", "gateway", "ota", "check"}, post("action_check"))
  entry({"admin", "gateway", "ota", "download"}, post("action_download"))
  entry({"admin", "gateway", "ota", "progress"}, call("action_progress"))
  entry({"admin", "gateway", "ota", "cancel"}, post("action_cancel"))
end
