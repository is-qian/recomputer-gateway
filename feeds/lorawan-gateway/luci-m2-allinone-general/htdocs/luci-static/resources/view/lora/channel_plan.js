...
function freq_config_render(region_cur) {
	...
	if (region_cur.indexOf("915") >= 0 || region_cur.indexOf("US915") >= 0 || region_cur.indexOf("AU915") >= 0
		...
		if( mode != "sensecap_ttn" && mode != "basic_station" && mode != "network_server" ) {
	...
	handleSaveApply: function (ev) {
		...
		setTimeout(function () {
			fs.exec('/etc/init.d/lora_pkt_fwd', ['restart']).then(
				...
				res => {
					if(  mode == "sensecap_ttn" ) {
						if (confirm(_('Restart the gateway configuration to take effect, Do you really want to Reboot?'))) {
							console.log("reboot")
							handleReboot();
						} 
