...
return view.extend({
	...
	render: function(data) {
		...
		for (var i = 0; i < wifidevs.length; i++) {
			...
			if (wifidevs[i].sid != null && wifidevs[i].sid.includes('default_radio')) {
				ifnameTitle = ifname.split('.')[0] + '.ap';
			} else if (wifidevs[i].sid != null && wifidevs[i].sid.includes('wifinet')) {
				ifnameTitle = ifname.split('.')[0] + '.sta';
			}
