...
return view.extend({
	...
	render: function(data) {
		...
		for (var i = 0; i < devs.length; i++) {
			var iftype = devs[i].getType();
			var ifname = devs[i].getName();

			if (!ifname)
				continue;

			if (iftype == 'bridge' || iftype == 'switch' || (iftype == 'vlan' && ifname.includes('eth0.1')))
				continue;

			var ifnameTitle = ifname;
			if (ifname.includes('wwan')) ifnameTitle = `${ifname} (cellular)`;
			else if (ifname.includes('wlan')) ifnameTitle = `${ifname} (wifi)`;

			var csvg = svg.cloneNode(true);

			v.firstElementChild.appendChild(E('div', { 'data-tab': ifname, 'data-tab-title': ifnameTitle }, [
				csvg,
